...
library(ggplot2)
library(ggrepel)
ggplot(dane_plot3,aes(x=SPEND_EDU,y=mean_all_by_CNT,color=continent))+geom_point(size=4)+stat_smooth(method = "auto",inherit.aes = FALSE,aes(x=SPEND_EDU,y=mean_all_by_CNT))+
  ggtitle("Średnia od wydatków na edukacje")+labs(y = "Średni wynik w kraju", x="Wydatki na eduakcje USD/Student")+theme(axis.title.x = element_text(size = 20),
