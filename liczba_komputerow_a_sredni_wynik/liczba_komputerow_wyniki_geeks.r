...
df <- select(dane, ST012Q06NA, PV1MATH:PV10SCIE) %>%
  filter(., !is.na(ST012Q06NA))
...
means_math <- group_by(df_math, ST012Q06NA) %>%
...
means_read <- group_by(df_read, ST012Q06NA) %>%
...
means_scie <- group_by(df_scie, ST012Q06NA) %>%
...
colnames(result) <- c("Computer_number","Mean_math", "Mean_reading", "Mean_science")

to_plot <- melt(result, id.vars = "Computer_number")

ggplot(to_plot, aes(x=Computer_number, y=value)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ variable) +
  ylab("Średni wynik") +
  ggtitle("Liczba komputerów w domu a wyniki najlepszych uczniów") +
