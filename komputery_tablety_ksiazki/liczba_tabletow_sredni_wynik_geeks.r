...
means_math <- group_by(df_math, ST012Q07NA) %>%
...
means_read <- group_by(df_read, ST012Q07NA) %>%
...
means_scie <- group_by(df_scie, ST012Q07NA) %>%
...
colnames(result) <- c("Tablet_number","Mean_math", "Mean_reading", "Mean_science")

to_plot <- melt(result, id.vars = "Tablet_number")

ggplot(to_plot, aes(x=Computer_number, y=value)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ variable) +
  ylab("Średni wynik") +
  ggtitle("Liczba tabletów w domu a wyniki najlepszych uczniów") +
