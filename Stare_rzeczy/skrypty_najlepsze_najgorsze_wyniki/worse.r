...
worse_in_all <- function(x, df){
  ...
  result <- result[order(result$average_pv_all, decreasing = FALSE),]
...
worse_in_science <- function(x, df){
  ...
  result <- result[order(result$average_pv_science, decreasing = FALSE),]
  return(result[1:floor(nrow(result)*(x/100)),])
}
...
worse_in_reading <- function(x, df){
  ...
  result <- result[order(result$average_pv_reading, decreasing = FALSE),]
  return(result[1:floor(nrow(result)*(x/100)),])
}
...
worse_in_math <- function(x, df){
  ...
  result <- result[order(result$average_pv_math, decreasing = FALSE),]
  return(result[1:floor(nrow(result)*(x/100)),])
}
